...
const listNS = async () => (await fetch(NS, { headers: AUTH })).json()

const origLevel = await getState(ITEMS.dimmer)
// section 4 picks themes, which the app saves to the shared settings; they stay in that browser
...
try {
  ...
  await section('1', async () => {
    ...
    const before = (await listNS()).length
    ...
    const after = (await listNS()).length
  ...
  await section('2', async () => {
    ...
    const server = await (await fetch(NS + '/dashboard:nh-corner-edit', { headers: AUTH })).json()
  ...
  await section('7', async () => {
    ...
    page.on('pageerror', (e) => errs.push(String(e.message)))
    await initToken(page, TOKEN)
    // a route only reaches new requests, so the open stream is stalled from inside instead, like a
    // half-open connection; the app's clock then jumps past the watchdog's 35s rather than waiting it out
    await page.addInitScript(() => {
      const Real = window.EventSource
      const realNow = Date.now.bind(Date)
      window.__nhStreams = []
      window.__nhSkew = 0
      Date.now = () => realNow() + window.__nhSkew
      window.EventSource = class extends Real {
        constructor(url, init) {
          super(url, init)
          this.__nhStalled = false
          if (String(url).includes('/rest/events/states')) window.__nhStreams.push(this)
        }
        addEventListener(type, fn, opts) {
          const self = this
          return super.addEventListener(type, function (e) { if (!self.__nhStalled) return fn.call(this, e) }, opts)
        }
        get onmessage() {
        ...
        set onmessage(fn) {
          const self = this
          super.onmessage = typeof fn === 'function' ? function (e) { if (!self.__nhStalled) return fn.call(this, e) } : fn
        }
      }
    })
    await page.goto(APP + '#/d/nh-corner-edit', { waitUntil: 'domcontentloaded' })
    ...
    await sleep(1500)
    const slider = async () => ((await page.locator('.nh-slider__value').textContent().catch(() => '')) ?? '').trim()
    const streams = () => page.evaluate(() => window.__nhStreams.length).catch(() => -1)

    await fetch(`${BASE}/rest/items/${ITEMS.dimmer}`, { method: 'POST', headers: { ...AUTH, 'Content-Type': 'text/plain' }, body: '21' })
    await sleep(1500)
    const v1 = await slider()
    ok('7. live update before outage', v1 === '21', v1)

    const opened = await streams()
    await page.evaluate(() => { for (const s of window.__nhStreams) s.__nhStalled = true })
    await fetch(`${BASE}/rest/items/${ITEMS.dimmer}`, { method: 'POST', headers: { ...AUTH, 'Content-Type': 'text/plain' }, body: '77' })
    await sleep(2500)
    const stalled = await slider()
    ok('7. a stalled stream delivers nothing, so only a reconnect can bring 77', stalled === '21', `shows ${stalled} with ${opened} stream(s) open`)

    await page.evaluate(() => { window.__nhSkew += 40_000 })
    let v2 = ''
    for (let i = 0; i < 25; i++) {
      await sleep(1000)
      v2 = await slider()
    ...
    const reopened = await streams()
    ok('7. the watchdog opened a new stream', opened >= 1 && reopened > opened, `streams ${opened} -> ${reopened}`)
  ...
  await sleep(1200)
  const leftover = (await listNS()).filter((c) => c.uid.includes('nh-corner'))
