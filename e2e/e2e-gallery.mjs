 ...
 * Widget gallery e2e.
 *
 * Covers: the bundled catalogue is served from the add-on itself (so it works with no internet and
 * no cross-origin permission), every entry's widget file loads and validates, installing one
 * creates a widgetdef component that then renders on a dashboard and appears in the palette,
 * re-installing an unchanged widget is a no-op, installing over a locally edited widget of the same
 * id lands beside it instead of overwriting, and an unreachable online catalogue is reported
 * without breaking the bundled list.
 *
 * SAFE with a live config: creates only widgetdef:gallery-* (+ any -2 copy) and
 * dashboard:nh-e2e-gal, deletes exactly those, and commands NOTHING.
 */
...
/** Anything gallery-related this suite might have created, cleaned by prefix. */
const cleanup = async () => {
...
try {
  /* ---------------- the catalogue is served by the add-on itself ---------------- */
  ...
  /* ---------------- re-installing an unchanged widget does nothing ---------------- */
  await card(first.name).locator('button').click()
  ...
  /* ---------------- installing over a local edit lands beside it ---------------- */
  ok(
  ...
  await openSettings()
  await card(first.name).locator('button').click()
  // Waited for, but not assumed: if the install overwrote instead, the check below has to be the
  // thing that fails, not a timeout that skips the rest of the suite.
  await page
  ...
  /* ---------------- an installed widget really works ---------------- */
  // put the pristine gallery version back, then render it on a dashboard
  ...
  /* ---------------- an unreachable online catalogue is reported, not fatal ---------------- */
  await openSettings()
  ...
  /* ---------------- a served online catalogue merges in ---------------- */
  ...
  // its file 404s: installing must report that, not create anything
