...
try {
  ...
  ok('the picker opens searching every set', (await page.locator('.nh-iconpicker__tab--on').textContent()) === 'All')
  await page.fill('.nh-iconpicker__search', 'thermometer')
  await sleep(800)
  // the point of the All tab: one word, results from wherever they live. Searching a single set is
  // what sent people back to the tabs to look for an icon they could not name the pack of.
  const packsHit = await page.$$eval('.nh-iconpicker__cell', (els) => [...new Set(els.map((e) => (e.title ?? '').split(':')[0]))].sort())
  ok('one search reaches more than one icon set', packsHit.length >= 2, JSON.stringify(packsHit))
  ok('including the mono set, which is not the one it opened on', packsHit.includes('mdi'), JSON.stringify(packsHit))
  await page.locator('.nh-iconpicker__cell[title="fluent:thermometer"]').first().click()
  await sleep(300)
  ok('picking a colour result sets its fluent ref', (await iconInput.inputValue()) === 'fluent:thermometer', await iconInput.inputValue())
  ...
  await sleep(600)
  const weatherOnly = await page.$$eval('.nh-iconpicker__cell', (els) => [...new Set(els.map((e) => (e.title ?? '').split(':')[0]))])
  ok('a tab still narrows to its own set', JSON.stringify(weatherOnly) === '["meteo"]', JSON.stringify(weatherOnly))
  await page.locator('.nh-iconpicker__cell[title="meteo:clear-night"]').first().click()
  await sleep(300)
