...
/**
 * Assembly - a dark green glass production board: translucent panels that blur what is behind
 * them (the page's own soft-lit green, or a background image when one is set), hairline mint
 * borders with an inner top light, vivid green instruments with brightened value tips, and
 * the active control drawn as the board's selected card - green outline, tinted fill, soft
 * glow. Type is Poppins (bundled; declared here so it only downloads with this theme).
 */
const ASSEMBLY_CSS = `@font-face {
  font-family: 'Poppins';
...
:root {
  ...
  --nh-good: #40d364;
  --nh-bad: #ef5350;
  /* the chart palette leads with the board's green, then its coral and blue */
  --nh-chart-1: #40d364;
  --nh-chart-2: #f0813c;
  --nh-chart-3: #3c62f0;
  /* light the solid-arc gauges: a bright film at the value tip, a sunk one at the start
     (the widget's band films default to invisible; these two opacities switch them on) */
  --nh-band-light: 0.5;
  --nh-band-shade: 0.28;
}
/* The room behind the glass: a defocused robot hall (bundled render - lit clerestory, rim-lit
...
.nh-widget:has(.nh-button--active) {
  border-color: color-mix(in srgb, var(--nh-primary) 70%, transparent);
  background-color: color-mix(in srgb, var(--nh-primary) 10%, rgba(30, 44, 36, 0.5));
  box-shadow:
    inset 0 1px 0 0 rgba(255, 255, 255, 0.07),
