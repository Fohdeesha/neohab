...
export async function exportComponent(
  ...
  try {
    ...
    if (out.missing.length > 0) {
      onNotice(
        i18n.t('Exported, but {{count}} referenced item(s) no longer exist and were left out: {{list}}', {
    ...
    onNotice(i18n.t('Export failed: {{error}}', { error: err instanceof Error ? err.message : String(err) }))
  }
}
