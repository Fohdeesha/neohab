 ...
 * it, or the command failed) does the display follow the live state again - and it snaps back
 * to the committed value if live later returns close to it.
 */
...
export function useOptimisticValue<T>(
  ...
  return { display, commit: (v: T) => setPending({ v, at: Date.now() }) }
}
